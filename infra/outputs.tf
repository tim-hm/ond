...
output "ssh_host" {
  description = "Where the deploy tasks and the restore command find the box: its MagicDNS name on the tailnet, not its public address, because 22/tcp is closed to the internet. Resolvable from any device on the tailnet and from nowhere else — a device that has drifted off the tailnet gets a DNS failure rather than a hang, which is the more legible of the two."
...
output "logs_bucket" {
  description = "Loki's chunk store. Rendered into infra/box/loki.yaml by `mise run deploy:api`, so the box learns it from this state rather than from a literal committed beside the config."
...
output "region" {
  description = "The region the box signs for. Read by `mise run deploy:api` to render the cron's AWS_DEFAULT_REGION and Alertmanager's sigv4 block, so neither carries a literal that could disagree with the provider."
  value       = var.region
}
