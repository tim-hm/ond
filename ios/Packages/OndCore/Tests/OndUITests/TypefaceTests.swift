...
/// The display face's failure mode is the palette's: silent. A missing or
/// renamed font file does not throw — `Font.custom` falls back to SF and the
/// wordmark quietly stops being the wordmark. So the file and the name the
/// code asks for are pinned here, against the source tree for `ColorSet`'s
/// reason: the test bundle's copy is a build-system artefact, and the source
/// is what ships.
