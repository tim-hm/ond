...
/// Starting your own exercise from a curated one. The copy is an ordinary
/// authored exercise that happens to open pre-filled — no lineage is kept and
/// nothing on the draft points back at the catalogue. What these pin is that the
/// content survives the trip and that the trip is offered only where it can be
/// made without quietly rewriting the exercise.
@Suite("Making your own version of a curated exercise")
struct TechniqueCopyTests {
    /// The limits the server would derive from this catalogue: fixed ceilings,
    /// with phase ranges derived from the seed rather than written down.
    /// `user_technique/repository.rs` takes the widest interval the seeded phases
    /// put each kind in, ignoring open-ended stages. Ranges typed into a fixture
    /// would pass while the seed moved underneath — the drift these tests catch.
    ...
    /// untouched: the composer shows the clamped draft, and the person reads
    /// those numbers as what they are about to get. The failure caught is a seed
    /// edit — widen a phase or add a fifth stage and copying silently returns
    /// something else. Whole-catalogue, so a new technique is covered on landing.
    ...
    /// withholds it rather than on the reason behind it — so a screen that offers a
    /// copy is offering exactly what this covers. A retention ends when the person
    /// does. Authoring cannot express that at all, so a copy would arrive with the
    /// defining feature of the protocol rewritten as a timed hold.
