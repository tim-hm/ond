...
/// a session runs. Every failure — no watch, no grant, out of range, a busy wrist
/// — is the same silence: `beatsPerMinute` is nil, unexplained. Follows the session
/// by observation, not screen calls: a backgrounded app stops evaluating view
/// bodies, and a view-ended arrangement would hold a workout open on the wrist.
@MainActor
@Observable
public final class PulseMonitor {
    ...
    /// Whether this session might show a rate at all — what a screen keeps a
    /// place for. `beatsPerMinute` comes and goes over a session, so the badge
    /// holds its height against this instead: settled at `follow(_:wanted:)`,
    /// fixed until the screen goes, and a session with no wrist coming reserves
    /// nothing.
    public private(set) var expectsReadings = false

    /// Every reading this session's wrist has sent — what the summary draws once
    /// the breathing is over. Deliberately outlives the arrangement that filled
    /// it: a rate is only true while arriving, but a record stays until the
    /// screen showing it goes, so it is cleared by [`follow(_:)`] and
    /// [`release()`] rather than by `end()`, which a finished session does call.
    ...
    /// How long a reading stands: two of the wrist's own sends
    /// (`PulseRelay.spacing`) and a little over, so a lost message does not
    /// blink the badge, and a removed wrist cannot leave a believable number on
    /// screen. `nonisolated` so `PulseTrace` breaks a line on the same figure —
    /// one definition keeps the blanked badge and the broken line agreeing.
    ...
    /// How far into its settle the invented heart has come. On the monitor
    /// rather than in the task, because a pause cancels the task: counted there,
    /// every resume would restart the curve at seventy-four. Reset per session
    /// by `follow(_:wanted:)`, beside the trace it feeds.
    ...
    /// Whether this monitor invents readings instead of asking a wrist. Taken
    /// rather than compiled in: which builds may invent a heart rate is the
    /// composition root's to answer — the deterministic UI-test harness runs in
    /// a simulator too and wants it off. `OndAppComposition.wristHandoff` is the
    /// one caller that ever passes true.
    ...
    ///   - rehearsing: see the property. Defaulted false so the honest
    ///     arrangement is what a caller gets by saying nothing.
    ...
    /// Ties the arrangement to one session's life: a pause or an interrupted
    /// countdown ends it like a finish does, and resuming arranges a fresh one.
    /// - Parameter wanted: whether the person asked for a heart rate. Answered
    ///   here because a rehearsing monitor follows either way — call sites would
    ///   otherwise need to know the preference is paywalled and simulators wristless.
    ...
    /// The wrist's answer to a sharing order. Only a no is worth acting on: a
    /// wrist that declines — a session breathed on it, or sharing under an older
    /// order — leaves this phone holding an arrangement nothing will answer.
    /// Ended here, the next session can arrange again; left standing, no session
    /// for the rest of the launch would ever show a badge.
    ...
    /// - Returns: false for nothing arranged, and for a reading shared under
    ///   some other order — a wrist finishing a previous session's arrangement.
    ///   Either way the wrist stops. The answer is the contract, so it is not
    ///   discardable.
    ...
    /// Tells the trace how long the session ran — the width the summary draws
    /// it across. Here rather than in `end()`, which a pause reaches too: fixing
    /// the width at a pause would leave a resumed session's readings drawn past
    /// the end of their own chart.
    ...
    /// Asks the wrist to start sharing. The context goes before the launch,
    /// because the launch says nothing: the watch app reads why from the last
    /// thing this phone said. A refused launch retracts the order — a spent
    /// errand left in the context rides every later push. An order the outbox
    /// will not carry — the pairing is önd+ — ends here, before the watch is woken.
    private func begin() {
    ...
    /// Ends the arrangement. Both halves cover different failures: the retraction
    /// — pushed, because a context already delivered is replayed on the watch's
    /// next activation — stops a wrist that has not opened yet, and the cleared
    /// arrangement answers the next reading of a wrist already sharing with a no.
    private func end() {
        if let ordered {
