...
public enum JourneyRepositoryError: LocalizedError, DiagnosticCarrying, Equatable {
    ...
    /// Everything the journey does over the network is optional, so the
    /// correct response is almost always to try again later. Carries the
    /// classified outcome and the transport's words — see [`TransportFault`].
    ...
    /// the local JSON files are plain files a person may edit, and every proto
    /// counter is unsigned 32-bit. Thrown rather than trapped, because
    /// `SessionSyncQueue.sync()` runs on every foreground and a trap there is
    /// a crash loop. The cost: the sync stops with the record at its head.
    case malformedRequest(String)

    /// Carries the associated message; without this conformance
    /// `localizedDescription` bridges to a bare `NSError` saying "The
    /// operation couldn't be completed". The two malformed cases say the one
    /// true thing and leave the detail to `diagnostic`.
...
/// archive: a single bounded call would hand back the most recent few and
/// call the restore done.
...
/// The network side of the journey: syncs and genuinely online reads.
/// Nothing the Progress tab draws about *this person* goes through it — that
/// is folded from the local stores, so the tab is complete with the radio
/// off. Leaderboards are the honest exception: other people need a connection.
public protocol JourneySyncing: Sendable {
    ...
    /// reinstall, where the Keychain identity outlived the local file. Asks
    /// for the sessions alone: the totals are the three heaviest reads the
    /// server does, and a restore folds its own and throws them away.
    /// - Parameter pageToken: nil for the newest page; the server's to mint.
...
public struct JourneyRepository: JourneySyncing {
    ...
    /// What one page of a restore asks for. Built apart from the call so a
    /// test can read it: `sessionsOnly` is unobservable through
    /// `JourneySyncing`, whose test doubles never see the message.
    ...
    /// How much history one restore page asks for — the server's own ceiling,
    /// so a restore costs the fewest round trips. Larger than anything a
    /// screen would ask: this call exists only for a device with no local
    /// store yet.
    ...
    /// The one distinction a caller acts on: whether anything but waiting
    /// could change the answer. A flag rather than a `Code` because Connect is
    /// OndAPI's dependency, not this target's. Every call tests it, so a
    /// server that begins refusing another RPC on a condition is not
