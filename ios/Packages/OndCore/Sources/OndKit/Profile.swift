...
/// How much the app is invited to ask for someone's attention. `never` is the
/// default in every direction — stored default, proto zero, and the fallback
/// for an unreadable preference — so nothing that goes wrong turns silence
/// into a notification. Permission is asked only when this moves off it. The
/// dial owns one schedule, `Schedule.fromDial`, reshaped by `ScheduleStore`.
...
/// The decade somebody was born in. Bands by birth decade rather than by age,
/// so a band never changes under a person. Optional everywhere. Both pickers
/// render `allCases`, so the youngest case is the youngest band önd offers and
/// must agree with the privacy policy and the App Store Connect age answers.
/// See `BirthYearBand` in `proto/ond/v1/profile_service.proto`.
...
/// What someone told the app about themselves. Answers, not derived state:
/// streaks and totals come from the sessions on this device, see
/// `JourneyStats`. `Codable` because it is kept locally as well as on the
/// server — onboarding has to finish with no network.
public struct Profile: Sendable, Equatable, Codable {
    ...
    /// them. Empty means invisible to everyone else. The server owns the final
    /// value — it trims, screens and suffixes a name somebody already holds —
    /// so what comes back from a sync may not be what was typed.
    ...
    /// What to call this person. Empty is the normal state, because the
    /// question is optional, so every greeting must read well without it.
    /// Empty rather than `nil`, following `displayName`. Unlike that one, this
    /// name is never printed on the boards, so the server neither screens nor
    /// suffixes it and what comes back is what was sent.
    ...
    /// Whether anybody has told this profile anything. The question a restore
    /// turns on: the server answers `GetProfile` for an identity never written
    /// to. Compared against the default wholesale rather than field by field,
    /// so a field added later is covered without extending a list.
    ...
    /// This profile narrowed to what the server will accept, and the one
    /// statement of the client's half of the validation rules. A field the
    /// client lets through and the server refuses leaves `UpdateProfile`
    /// returning `INVALID_ARGUMENT` on every launch, taking the whole profile
    /// with it. The note is bounded but not stripped: a newline is a paragraph.
...
extension String {
    ...
    /// validation and the database `CHECK` both count. A grapheme-cluster
    /// count would let through multi-scalar emoji the server then rejects. The
    /// UTF-8 length is checked first because it is O(1) and never smaller.
    ...
    /// then cut to `limit` Unicode scalars. Stripped as somebody types rather
    /// than refused, so a pasted name arrives cleaned. Category Cc exactly,
    /// matching Rust's `char::is_control`; `CharacterSet.controlCharacters` is
    /// wider and would take the zero-width joiner inside an emoji.
