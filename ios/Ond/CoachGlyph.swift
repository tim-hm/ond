...
///
/// Five surfaces draw it — the tab, the coach's empty room, its offer, the door
/// on an exercise's own screen, and the paywall's benefit row — and until this
/// existed each held its own string literal with a comment asking the next
/// person to keep them in step. Four of the five are in different features, so
/// a retune reached them only if whoever made it read the comment.
///
