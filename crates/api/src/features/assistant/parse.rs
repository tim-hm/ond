...
//! willing to name. Its own file rather than the back half of `super::prompt`:
//! that module decides what to ask for, this one decides what to believe, and
//! only this one is load-bearing for safety.
...
/// Turns a model's reply into slugs this server is prepared to name — the
/// guard, not a formality. Everything is dropped that is not a `slug | reason`
/// line naming a catalogue technique; duplicates collapse. An empty result is
/// the expected answer to an unusable reply, read as "fall back to the rules"
/// — so no unvalidated text can be mistaken for a slug or reach a client.
