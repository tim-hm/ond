...
/// doubling every error count taken off the log. What it adds instead is any
/// `grpc_status` already present in the response head. A streaming call's final
/// status belongs to the completion metric, because it arrives later.
///
