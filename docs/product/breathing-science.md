...
- **Safety**: none on the technique — deliberately. The red-flag triage rides `when-youre-winded` instead (§3.13 and standing rule 3), which keeps the technique-note set at the pinned two.
...
### 3.13 The occasions that ship with it
...
**Shipped since this list was drafted**: humming breath, and all four proposed occasions — `riding-out-a-craving`, `when-you-cant-get-a-satisfying-breath`, `in-a-tight-spot` and `feeling-queasy`. Their patterns, claims, citations, refusals and safety facts now live in §3.12 and §3.13. The two paragraphs below have shipped since too, so nothing in Tier 2 remains undecided but the mouth-taping refusal the page had no words for.
...
- **`light-and-slow`** (panic candidate; technique) — nose 2.5s in (2–3.5) · nose 4.5s out (3.5–6.5), ~26 cycles, ~9 bpm with deliberately _small_ breaths. The trialled panic-adjacent shape (§6.1), outside every existing dial floor. Against: a second nose-ratio figure in the catalogue, and an evidence paragraph that must admit the mechanism is contested. If it ships: occasion `when-panic-is-rising`, FullScreen, 180s. If not, the "deep breath" foundations correction carries most of the value.
