...
**Two of the four are enforced on the device only, and that is a decision.** The rule cuts both ways: a health trend is read off a local daemon and a session sent to the wrist crosses `WCSession` between two devices somebody already owns, so on neither path is there a server call to refuse. They are sold because they are worth paying for and cost real engineering, not because they cost per use — and for those two the paywall is a checkout rather than a security boundary. A patched build takes them and nothing would notice, which is acceptable precisely because taking them costs us nothing. The two that do cost — the coach and the boards — are the two the server refuses, and no client can talk it out of either.
