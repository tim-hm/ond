...
**mise redaction breaks stdin.** `.env` must not carry `redact = true`. mise implements redaction by capturing every task's stdio in order to filter it, and a captured stdin reaches the child already closed — a property of the whole file, not of one task. It cost us `db:psql` reading no statements and exiting 0, and `tofu apply` dying of "error asking for approval: EOF" on the prompt that is the entire safety of an apply. What it would buy is nothing: the file holds Apple identifiers, while the one real credential — the `.p8` — lives outside the repository where redaction never covered it.
