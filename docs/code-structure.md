...
Integration tests are the exception, because they belong to no single file. `crates/api/tests/e2e/` mirrors the feature layout one level up, and escalates on the same rule as the tiers above. A suite stays one file while it covers one concern — `technique.rs` for that feature, `profile.rs` for its own, `health.rs` for the JSON surface. Once it covers several it becomes a directory: a `mod.rs` of `mod` declarations and nothing else, a `fixtures.rs` holding what every test in the suite arranges before it calls anything, and one file per concern. Each concern file names what it uses — `use super::fixtures::{…}` for the suite's own arrangements, `use crate::harness::{…}` for anything shared — so a reader can tell the two apart without opening either. `account/`, `assistant/`, `entitlement/`, `journey/` and `user_technique/` are all that shape. `harness/` holds everything more than one suite needs: the disposable database, the production router, the deterministic doubles, the gRPC-Web client, the metrics scrape, one definition of every RPC path, and the domain arrangements more than one suite writes rows with (`given_user`, `sign_in`, `subscribe`, `record`). Look there before writing a second one. Cargo treats `tests/e2e/main.rs` as one target, so all of them compile into a single binary rather than one per file.
