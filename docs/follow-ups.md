...
**A discreet session passes no safety warning, however it started.** The wrist gates its guided sessions on `TechniqueWarningStore`. `DiscreetSessionView` does not, whether it was opened from the wrist's own Moments list or ordered by the phone: its whole point is a screen nobody looks at. No seeded discreet occasion carries a note today — the six run `coherent-breathing`, `extended-exhale` or `box-breathing` — so nothing is currently unsaid. The first warned discreet occasion breaks that silently. _Closes when a discreet start either refuses a warned exercise or shows the warning before the screen goes dark._
