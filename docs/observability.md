...
- A **Route 53 health check** probes `https://api.ondbreathe.app/health` from the internet and alarms into the same SNS topic. The API's name rather than the apex, because the API is what an installed app depends on; both resolve to the same Caddy, so a box-level failure still fails the check, but a static site broken while the API is fine would not be caught. It matches on the response body rather than the status, because a 200 on 443 is not a claim that the API answered — whatever takes that port answers something, and Caddy holding no certificate for a name still serves its own default. Its CloudWatch alarm lives in **us-east-1**, which is the only region Route 53 publishes those metrics to — an alarm built anywhere else sits in `INSUFFICIENT_DATA` for ever.
